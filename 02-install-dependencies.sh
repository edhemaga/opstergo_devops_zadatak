...
MINIKUBE_INSTALLED=$(check_minikube)

if [ $DOCKER_INSTALLED -eq 0 ] && [ $MINIKUBE_INSTALLED -eq 0 ]; then
    echo "Prerequisites installed! All good to go..."
    exit 0
fi
...
if [ $OS_TYPE == 1 ]; then
    ...
    if [ $MINIKUBE_INSTALLED -eq 1 ] && [ $CPU_ARCH_TYPE -eq 1 ]; then 
